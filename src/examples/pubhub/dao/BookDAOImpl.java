...
public class BookDAOImpl implements BookDAO {
	...
	public List<Book> getAllBooks() {
		System.out.println("BookDAOimpl.java Line 27");
		
		List<Book> books = new ArrayList<>();
System.out.println("BookDAOimpl.java  Before connection");
		try {
			System.out.println("BookDAOimpl.java  After Try");
			connection = DAOUtilities.getConnection();	// Get our database connection from the manager
			System.out.println("BookDAOimpl.java  After connection");
	...
	public Book getBookByISBN(String isbn) {
		System.out.println("BookDAOimp ISBN =" + isbn);
		Book book = null;
		...
		try {
			...
			stmt = connection.prepareStatement(sql);
			System.out.println("BookDAOimp Connect to DB");
			stmt.setString(1, isbn);
			
			ResultSet rs = stmt.executeQuery();
			System.out.println("BookDAOimp RS = " + rs);
			if (rs.next()) {
				System.out.println("BookDAOimpl ISBN =" + isbn);
				book = new Book();
				book.setIsbn13(rs.getString("isbn_13"));
				System.out.println("BookDAOimpl get isbn_13");
				book.setAuthor(rs.getString("author"));
				System.out.println("BookDAOimpl get author");
				book.setTitle(rs.getString("title"));
				System.out.println("BookDAOimpl get title");
				book.setPublishDate(rs.getDate("publish_date").toLocalDate());
				System.out.println("BookDAOimpl publish date");
				book.setPrice(rs.getDouble("price"));
				System.out.println("BookDAOimpl setPrice");
				book.setContent(rs.getBytes("content"));
				System.out.println("BookDAOimpl content");
			}
			
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("BookDAOimpl Exception");
		} finally {
			closeResources();
			System.out.println("BookDAOimpl Close Resource");
		}
	...
	@Override
	public boolean getBookTagByISBN(String isbn) {
		System.out.println("BookDAOimp bookFound ISBN =" + isbn);
		Book book = null;
		...
		try {
			...
			stmt = connection.prepareStatement(sql);
			System.out.println("BookDAOimp bookFound Connect to DB ");
			stmt.setString(1, isbn);
			
			ResultSet rs = stmt.executeQuery();
			System.out.println("BookDAOimp bookFound RS = " + rs);
			if (rs != null) {
				bookFound = true;
				System.out.println("BookDAOimp bookFound Book found");
				
			}
			
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("BookDAOimpl bookFound Exception");
		} finally {
			closeResources();
			System.out.println("BookDAOimpl bookFound Close Resource");
		}
